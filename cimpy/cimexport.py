        ...
        if 'value' in attributes.keys():
            if isinstance(attributes['value'], list):
                for reference_item in attributes['value']:
                    ...
                    if reference_item not in ['', None]:
                        reference_list.append({'value': reference_item, 'attr_name': key})
            # ignore default values
            # elif attributes['attr_name'] in iterkeys: # added by mohamed
            #     reference_list.append(attributes)
            elif isinstance(attributes['value'], ( bool)): # added by Mohamed, because PF doesnt accept True or False
                    if attributes['value']==True :
                        attributes['value']="true"
                    elif attributes['value']==False :
                        attributes['value']="false"
                    reference_list.append(attributes)
            elif attributes['value'] not in ['', None, 'list']: # deleted 0 and 0.0 in the list because PF needs also the variable setted as 0 (Mohamed)
                reference_list.append(attributes)
...
def _set_attribute_or_reference(text, render):
    ...
    value = result[0]
    attr_name = result[1]
    #if else added by mohamed : some attr dont have cim as name spase.Temporary solution#added attr_ns by mohamed. .mustache file was also modified
    if "entsoe" in value or "IdentifiedObject.shortName"== attr_name:
        attr_ns="entsoe"
    else:
        attr_ns="cim"
    if '%URL%' in value:
        reference = value.split('%')[2]
        return attr_ns+":"+attr_name+' rdf:resource="' + reference + '"/>'
        #return ' rdf:resource="' + reference + '"/>'
    elif '%' in value:
        reference = value.split('%')[1]
        return attr_ns+":"+attr_name+' rdf:resource="#' + reference + '"/>'
    else:
        return attr_ns+":"+attr_name +'>'+  value + '</'+attr_ns+':' + attr_name + '>'
...
def _set_attribute_or_reference_model(text, render):
    ...
    if '%' in value:
        reference = value.split('%')[1]
        return ' rdf:resource="' + reference + '"/>'
    elif 'urn' in value: #added by mohamed
        return ' rdf:resource="' + value + '"/>'
    else:
...
def generate_xml(cim_data, version, model_name, profile, available_profiles):
    ...
    #Model header
    model_description = {
        'mRID': cim_data["meta_info"]["profile_about"][profile.name],
        'description': [
    ...
    if "author" in cim_data["meta_info"] and (cim_data["meta_info"]["author"] != None):#added by Mohamed to add correct author if existed
        model_description['description'].append({'attr_name': 'modelingAuthoritySet',
             'value': cim_data["meta_info"]["author"]})
    else:
        model_description['description'].append({'attr_name': 'modelingAuthoritySet',
             'value': "FAU"})
    #added by Mohamed to display multiple profiles (in case of EQ)
    for profile_name in cim_data["meta_info"]["profiles"][profile.name]:
        model_description['description'].append({'attr_name': 'profile',
        'value': profile_name})


    profiledepON={"TP":["EQ"],"SV":["TP","SSH"],"SSH":["EQ"],"DY":["EQ"],"DL":["EQ","TP"],"GL":["TP","EQ"]}#Mohamed -- in case no depon

        #added by Mohamed to display model dependon
    if 'DependentOn' in cim_data["meta_info"]["profile_about"] :
        if profile.name in cim_data["meta_info"]["profile_about"]['DependentOn']:
            for key in cim_data["meta_info"]["profile_about"]['DependentOn'][profile.name].keys():
                for val in cim_data["meta_info"]["profile_about"]['DependentOn'][profile.name][key]:
                    dependentOndict=[{'attr_name': 'DependentOn', 'value': val}]
                    model_description['description']+=dependentOndict
    else:
        for key in profiledepON.keys():
            if profile.name == key:
                for value in profiledepON[key]:
                    dependentOndict=[{'attr_name': 'DependentOn', 'value': cim_data["meta_info"]["profile_about"][value]}]
                    model_description['description']+=dependentOndict



    #print(cim_data['meta_info']['urls'])
