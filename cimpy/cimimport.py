...
def cim_import(xml_files, cgmes_version, start_dict=None):
    ...
    # create sub-dictionaries #Added by Mohamed profiles={} profile_about
    import_result['meta_info'] = dict(namespaces=_get_namespaces(xml_files[0]), urls={},profiles={},profile_about={})
...
def _instantiate_classes(import_result, xml_files, cgmes_version_path, namespace_rdf, base,
    ...
    for xml_file in xml_files:
        ...
        package = ''
                ...
                #added by mohamed so if dependentOn is before profile an error wont be raised
        for pck, package_key in short_package_name.items():        
            if package_key in str(xml_file): 
                package = pck
        #print(package)
        for event, elem in context:
            ...
            # Check which package is read #This is for the Fullmodel data
            elif event == "end":
                ...
                if 'Model.profile' in elem.tag:
                    for package_key in short_package_name.keys():
                        ...
                        if package_key in elem.text:
                            ...
                            if short_package_name[package_key] in import_result['meta_info']['profiles']:
                                import_result['meta_info']['profiles'][short_package_name[package_key]].append(elem.text)
                            else:
                                import_result['meta_info']['profiles'][short_package_name[package_key]]=[elem.text]
                            package = package_key
                            break
                ...
                if 'Model.DependentOn' in elem.tag:                 
                    ...
                    if short_package_name[package] not in import_result['meta_info']['profile_about']['DependentOn']:
                        import_result['meta_info']['profile_about']['DependentOn'][short_package_name[package]]={}
                        import_result['meta_info']['profile_about']['DependentOn'][short_package_name[package]]["resource"]=[]
                    import_result['meta_info']['profile_about']['DependentOn'][short_package_name[package]]["resource"].append(elem.attrib.values()[0])
            ...
            if event == "start" and elem.tag[:m] == base :
                ...
                if uuid is not None:
                    ...
                    for event, elem in context:
                        ...
                        if event == "end" and (elem.tag[:m] == base or "entsoe" in elem.tag ):#"entsoe" in elem.tag  added by mohamed
                            # Break if class closing element (e.g. </cim:Terminal>).
                            if elem.get("{%s}ID" % namespace_rdf) is None \
                                    and elem.get("{%s}about" % namespace_rdf) is None: #Mohamed , this filters the <entsoe:OperationalLimitType.limitType ... 3/1#LimitTypeKind.patl"
                                ...
                                if uuid2 is None:  # attribute
                                    ...
                                    try:
                                        ...
                                        if isinstance(getattr(obj, attr), bool):  # if typ==<class 'bool'>
                                            ...
                                            setattr(obj, attr, elem.text) #setattr(obj, attr, typ(elem.text)); Modified by mohamed so that the original 0 setted values wont be filterred in the export 
                                    except TypeError:
                                        try:
